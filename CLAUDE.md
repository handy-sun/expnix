...
modules/            → Custom reusable modules (sing-box with configGeneration)
...
isDarwin, isWSL, isHeLinux (= !isDarwin && !isWSL), isHmSingle
```
...
- Conditional packages: `isHmSingle` → `rustc + cargo`; otherwise → `rust-bin.stable.latest.default`
